...
public class CustomEventListenerProvider
    ...
    // Convert user attributes to JSON-like string
    ...
    attributes.forEach((key, value) -> {
        attributesJson.append("\"").append(key).append("\": \"").append(value).append("\",");
    });
    if (attributesJson.length() > 1) {
        attributesJson.setLength(attributesJson.length() - 1); // Remove trailing comma
    }
    attributesJson.append("}");

    // Construct the main JSON string including the attributes
    String data = "{" +
                  "\"id\": \"" + user.getId() + "\"," +
                  "\"email\": \"" + user.getEmail() + "\"," +
